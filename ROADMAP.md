...
- **Confused deputy enforcement** — sub-agent output treated as opaque strings in orchestrator; never eval'd, JSON-parsed for tool calls, or passed as raw system prompt content without wrapping. **✅ Its regression test exists and passes** — `tests/run_b1_redteam.py --suite deputy`, re-run 2026-09-03 (`tests/security_redteam_2026-09-03_deputy.md`). Confirmed while working `[DB-0804-02]`, which had carried it as outstanding. **✅ Re-run 2026-09-24, PASS 2/2** (`tests/security_redteam_2026-09-24_deputy.md`) after `tools/subagent.py`'s recursion guard became depth-aware (`MAX_SUBAGENT_DEPTH = 2`) for the four-layer graph. **Worth knowing before anyone reads the guard as this control:** it is not. The protection is architectural — `_dispatch_from_coordinator()` is the only function parsing free text into dispatch calls and its only call site is fed `coord_output` — which is exactly why a category router must fan out by *calling* `run_subagent`. A second dispatch parser reading agent output would break this check directly.
