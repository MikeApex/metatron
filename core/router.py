...
def _load_routing() -> dict:
    """
    The routing config for this deployment mode.

    Build's overlay merge was REMOVED 2026-09-24 with the overlay itself (build
    plan v4.11 ruling 4, § 6). A generated capability is now an ordinary TRACKED
    entry in this file — written into both routing files in the same diff as its
    code, read by Mike, and committed and deployed like every hand-written one —
    so there is nothing left to merge at load time. All four of Build's load
    seams are retired with it. `resolve_model()` and `get_allowed_tools()` below
    never needed to know about the overlay, and still do not.
    """
