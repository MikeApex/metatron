...
*Updated: 2026-08-03 (2nd window) — **check-in restraint live; the VM now formally owns persona config; biographical capture built.** Check-ins are activity-gated (60m quiet / 180m floor) and the over-long check-in prompt is fixed at the template. `write_profile`/`read_profile` capture facts the user gives in conversation, with contact details deliberately kept out of every prompt. `deploy.sh` must never push persona config — the VM writes it at runtime. Deployed through `35e53ee`. **Rule Redundancy plan agreed but NOT started — live duplicate debt, see below.** Previously this window: —*
...
**⚠ NOT STARTED — Rule Redundancy plan (agreed this session).** Live debt: the user's five complaints sit in **both** `mike.md` (Synth self-applied them) and `synthesizer.md` (added the same afternoon); nothing detected the overlap. Four items: (1) **repeat-detection** — a repeated instruction is evidence a rule isn't working, not a new rule, and is what would have caught *"once again, you're making too much of the sleep disruption"*; (2) one home per rule class, documented and checkable; (3) promotion deletes the original; (4) a reconciliation script reporting overlaps and contradictions. **Start with 1 and 3.**
