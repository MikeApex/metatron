...
*Updated: 2026-08-03 (4th window, final close — addendum; the 5th-window entry below folded in this window's earlier state and predates these two items) — **`deploy.sh` cried wolf on a good deploy, and is fixed.** Its new assertion tested **exact HEAD equality**, so when a parallel window pushed between this window's push and the VM's pull, the VM was strictly *ahead* — running the pushed commit plus one more — and it printed `DEPLOY FAILED … running OLD CODE`. The opposite of true. It now tests **ancestry** (`git merge-base --is-ancestor`) with four distinct outcomes: `unverified` / `match` / `ahead` (success, and it **names the extra commits** so an unexpected parallel deploy is not hidden) / `failed`. Ancestry and the extra-commit log come back in the same SSH call. **The `ahead` branch is harness-tested only** — one window cannot stage a parallel push, so it has never run against a live VM. Stated because the failure it guards against is silence: *an alarm that cries wolf on a good deploy is one people learn to ignore.* Also: the external-IP pricing is now **verified against the Cloud Billing Catalog API** rather than memory — `External IP Charge on a Standard VM` and `Networking Cloud NAT IP Usage` are **both $0.005/hr**, so Cloud NAT cannot be a saving on any usage assumption; real cost ~$3.65/mo, nil while paused, and the money is the $24.50 e2-medium line. Deployed `3492d42`, `c674a91`.*
