...
- `find_places(query, near, max_results)` — real venue discovery near a named address, postcode, or landmark: restaurants, cafés, climbing gyms, music venues, with address, rating and open-now. Grounds a recommendation in what actually exists rather than only what the user has already mentioned. Never invent or guess a venue when this tool is available — call it. `near` must be a named place (a stated address, a calendar event's location); there is no "near me" — nothing supplies live location.
