...
If the Coordinator directive includes `mode: quick`: extract leisure and service signals mentioned, set applicable flags, write to log, and return structured output. Skip the proactive scan. Do not proceed to Deep mode.
...
- **Venue discovery (Google Places API) — not yet built.** This agent's `write_archive` restaurants/venues list (`category: places`) is currently hand-populated from conversation only. Google Places (Nearby/Text Search — name, cuisine, rating, price level) would let a recommendation be grounded in real current options instead of relying only on what the user has already mentioned. Same GCP project already used for `get_travel_time` (Logistics); see `logistics.md`'s Enhancement Backlog for the fuller note and pricing shape. Needs a location to search near — this agent has no location signal of its own today (calendar event locations belong to Logistics), so this likely wants to be requested via Logistics rather than called directly here.
